defmodule BeamWeaver.Runnable.Config do
  ...
  def normalize(opts) when is_list(opts) do
    ...
    trace = Keyword.get(opts, :trace, base.trace)

    configurable =
      base.configurable
      |> merge_map_opt(opts, :configurable)
      |> copy_top_level_configurable(opts)
      |> configurable_with_trace_thread_id(trace)
  ...
  def inheritable_metadata(config_or_opts) do
    ...
    |> TraceOptions.metadata(config.trace)
  end

  defp from_map(map) do
    trace = Map.get(map, :trace, Map.get(map, "trace"))
  ...
  defp configurable_with_trace_thread_id(configurable, trace) do
    %{configurable: configurable}
    |> TraceOptions.put_thread_id_config(trace)
    |> Map.get(:configurable, configurable)
  end
