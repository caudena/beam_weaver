defmodule BeamWeaver.Graph.Errors do
  ...
  BeamWeaver keeps recoverable graph failures as tagged
  `%BeamWeaver.Core.Error{}` values and uses this module only to centralize
  graph error-code metadata and common constructors.
  """
  ...
  @doc """
  Appends the BeamWeaver graph error code.
  """
  ...
  def create_message(message, error_code) when is_binary(message) do
    ...
    message <> "\nBeamWeaver graph error code: " <> code
  end
  ...
  def node_timeout(node, elapsed, opts) when is_map(opts) do
    kind = opts |> Map.get(:kind, Map.get(opts, "kind")) |> normalize_timeout_kind()
    ...
    timeout = if kind == :idle, do: idle_timeout, else: run_timeout
  ...
  defp node_timeout_message(node, elapsed, :idle, timeout) do
  ...
  defp normalize_timeout_kind(nil), do: :run
  defp normalize_timeout_kind(:run), do: :run
  defp normalize_timeout_kind(:idle), do: :idle

  defp normalize_timeout_kind(kind),
    do: Options.atom_enum!("timeout kind", kind, [:idle, :run])
