defmodule BeamWeaver.Agent.Middleware.ToolRetry do
  ...
  defp normalize_on_failure(value) when value in [:error, "error"], do: :error
  defp normalize_on_failure(value) when value in [:continue, "continue"], do: :continue
  defp normalize_on_failure(value) when value in [:raise, "raise"], do: :error

  defp normalize_on_failure(value) when value in [:return_message, "return_message"],
    do: :continue

  defp normalize_on_failure(fun) when is_function(fun, 1), do: fun

  defp normalize_on_failure(value) do
    raise ArgumentError, "invalid tool retry on_failure: #{inspect(value)}"
  end
