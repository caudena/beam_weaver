defmodule BeamWeaver.Agent.Middleware.OverflowRecovery do
  ...
  def derive_threshold_tokens({kind, value}, _max_input_tokens)
      when kind in [:tokens, "tokens"] and is_integer(value),
      do: value

  def derive_threshold_tokens({kind, value}, max_input_tokens)
      when kind in [:fraction, "fraction"] and is_number(value) do
