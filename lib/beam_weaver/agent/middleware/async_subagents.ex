defmodule BeamWeaver.Agent.Middleware.AsyncSubagents do
      ...
      {updated, entries} =
        Enum.reduce(filtered, {%{}, []}, fn task, {tasks_acc, entries} ->
...

          {
            ...
            [
              "- task_id: #{task_id}  agent: #{task_value(live_task, :agent_name) || task_value(live_task, :subagent_name)}  status: #{task_value(live_task, :status)}"
              | entries
            ]
          }
        end)
  ...
  defp task_subagent(%__MODULE__{subagents: subagents}, task) do
    name = task_value(task, :agent_name) || task_value(task, :subagent_name)
