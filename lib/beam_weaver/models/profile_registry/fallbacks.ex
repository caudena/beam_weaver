defmodule BeamWeaver.Models.ProfileRegistry.Fallbacks do
  ...
  defp anthropic_max_input_tokens(model) do
    if high_context_anthropic_model?(model) do
      1_000_000
    else
      200_000
    end
  ...
  defp anthropic_max_output_tokens(model) do
    if high_output_anthropic_model?(model), do: 128_000, else: 64_000
  end

  defp anthropic_extra(model) do
    if adaptive_only_anthropic_model?(model) do
  ...
  defp high_context_anthropic_model?(model) do
    high_output_anthropic_model?(model) or String.contains?(model, "sonnet-4-6")
  end

  defp high_output_anthropic_model?(model) do
    opus_4_minor_at_least?(model, 6) or
      String.contains?(model, ["fable-5", "mythos-5"])
  end

  defp adaptive_only_anthropic_model?(model) do
    opus_4_minor_at_least?(model, 7) or
      String.contains?(model, ["fable-5", "mythos-5"])
  end
