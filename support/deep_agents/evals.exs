defmodule BeamWeaver.TestSupport.DeepAgents.Evals do
  ...
  @model_groups %{
    ...
    "frontier" => ["anthropic:claude-opus-4-7", "openai:gpt-5.5", "openai:gpt-5.5-pro"],
    "openai" => [
      "openai:gpt-4.1",
      "openai:gpt-5",
      "openai:gpt-5-mini",
...
defmodule BeamWeaver.TestSupport.DeepAgents.Evals.LangSmith do
  @moduledoc "Optional LangSmith eval helper boundary."

  @api_url "https://api.smith.langchain.com"

  def api_url, do: BeamWeaver.Config.get([:langsmith, :endpoint], @api_url)
  def resolve_langsmith_api_key, do: BeamWeaver.Config.get([:langsmith, :api_key])

  def create_example_id_from_instruction(instruction) do
    :crypto.hash(:sha256, to_string(instruction))
    |> Base.encode16(case: :lower)
  end

  def ensure_dataset(name, opts \\ []), do: create_dataset(name, opts)

  def create_dataset(name, opts \\ []) do
    langsmith_result(%{"name" => name, "metadata" => Map.new(opts)})
  end

  def create_experiment(dataset, opts \\ []) do
    langsmith_result(%{"dataset" => dataset, "metadata" => Map.new(opts)})
  end

  def create_experiment_async(dataset, opts \\ []),
    do: Task.async(fn -> create_experiment(dataset, opts) end)

  def add_feedback(run_id, key, score, opts \\ []) do
    langsmith_result(%{
      "run_id" => run_id,
      "key" => key,
      "score" => score,
  ...
  defp langsmith_result(payload) do
    case resolve_langsmith_api_key() do
      nil -> {:error, :missing_langsmith_api_key}
      _key -> {:ok, Map.put(payload, "api_url", api_url())}
    end
  end
end

defmodule BeamWeaver.TestSupport.DeepAgents.Evals.LangSmithEnvironment do
  @moduledoc "LangSmith-style environment wrapper over a BeamWeaver sandbox."

  defstruct sandbox: nil, internet_disabled?: false, gpu?: false, mounted?: false

  def new(opts \\ []), do: struct(__MODULE__, Map.new(opts))
  def type(_env), do: "beam_weaver_sandbox"
  def supports_gpus(%__MODULE__{gpu?: gpu?}), do: gpu?
  def can_disable_internet(_env), do: false
  def is_mounted(%__MODULE__{mounted?: mounted?}), do: mounted?
  def start(%__MODULE__{} = env), do: {:ok, env}
  def stop(%__MODULE__{} = _env), do: :ok

  def exec(%__MODULE__{sandbox: sandbox}, command, opts \\ []),
    do: BeamWeaver.Sandbox.execute(sandbox, command, opts)

  def upload_file(%__MODULE__{sandbox: sandbox}, path, content, opts \\ []),
    do: BeamWeaver.Sandbox.write(sandbox, path, content, opts)

  def download_file(%__MODULE__{sandbox: sandbox}, path, opts \\ []),
    do: BeamWeaver.Sandbox.read(sandbox, path, opts)

  def upload_dir(%__MODULE__{} = env, files, opts \\ []),
    do: Enum.map(files, fn {path, content} -> upload_file(env, path, content, opts) end)

  def download_dir(%__MODULE__{sandbox: sandbox}, path, opts \\ []),
    do: BeamWeaver.Sandbox.glob(sandbox, "**/*", Keyword.put(opts, :path, path))
end

defmodule BeamWeaver.TestSupport.DeepAgents.Evals.Harbor do
  @moduledoc "Optional Harbor/LangSmith eval integration boundary."
