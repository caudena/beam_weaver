defmodule BeamWeaver.Cache.ETSTest do
  ...
  test "ETS cache clear without namespace clears all entries" do
    # Upstream reference:
    # langchain/libs/core/tests/unit_tests/caches/test_in_memory_cache.py
    cache = Cache.ETS.new(visibility: :private)
  ...
  test "batch cache facade maps LangGraph full keys to explicit adapter operations" do
    # Upstream references:
    # - langgraph/libs/checkpoint/langgraph/cache/base/__init__.py
    # - langgraph/libs/checkpoint/langgraph/cache/memory/__init__.py
    cache = Cache.ETS.new(visibility: :private)
    key_a = {{"graph", "node"}, "a"}
    key_b = {{"graph", "other"}, "b"}
    key_c = {{"graph", "node"}, "c"}
  ...
  test "ETS cache evicts oldest entries when max_entries is reached" do
    # Upstream references:
    # - langchain/libs/core/tests/unit_tests/caches/test_in_memory_cache.py
    # - langchain/libs/standard-tests/langchain_tests/integration_tests/cache.py
