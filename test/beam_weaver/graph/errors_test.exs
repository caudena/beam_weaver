defmodule BeamWeaver.Graph.ErrorsTest do
  ...
  test "graph error codes and messages use tagged BeamWeaver errors" do
    ...
    assert message =~ "BeamWeaver graph error code: GRAPH_RECURSION_LIMIT"
  ...
  test "node timeout helper preserves timeout context" do
    ...
    assert_raise ArgumentError, ~s(timeout kind must be an atom, got "idle"; use :idle), fn ->
      Errors.node_timeout(:worker, 1.25, kind: "idle", idle_timeout: 1.0)
    end
  end

  test "graph failure policy rejects string enum values" do
    assert Options.normalize_failure_policy(:proceed) == :proceed

    assert_raise ArgumentError, ~s(failure_policy must be an atom, got "proceed"; use :proceed), fn ->
      Options.normalize_failure_policy("proceed")
    end
  end
end
