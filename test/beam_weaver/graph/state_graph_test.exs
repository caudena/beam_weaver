defmodule BeamWeaver.Graph.StateGraphTest do
  ...
  test "add_sequence wires ordered nodes without requiring Python-style builders" do
    # Translates LangGraph StateGraph.add_sequence behavior from tests/test_state.py.
    graph =
      Graph.new()
      |> Graph.add_sequence([
  ...
  test "waiting edges run a node only after every upstream node has completed" do
    # Translates LangGraph waiting-edge fan-in behavior from tests/test_state.py.
    graph =
      Graph.new()
  ...
  test "node defer metadata is inspectable without changing the public scheduler contract" do
    # Upstream reference:
    # langgraph/tests/test_state.py deferred-node metadata/rendering coverage.
    graph =
