defmodule BeamWeaver.Tools.PolicyToolsTest do
  ...
  # Upstream reference:
  # - langchain/libs/langchain_v1/tests/unit_tests/agents/middleware/implementations/test_shell_tool.py
  ...
  test "shell policy validates execution controls" do
    # Upstream reference:
    # langchain/libs/langchain_v1/tests/unit_tests/agents/middleware/implementations/test_shell_execution_policies.py
    assert {:error,
            %{
  ...
  test "TODO tool updates explicit state through a graph command" do
    ...
    messages = [
      Message.assistant("",
        tool_calls: [
          %{id: "call-todo", name: "todo", args: %{"action" => "add", "text" => "ship"}}
        ]
      )
    ]
    ...
    assert [%{id: id, text: "ship", status: "open"}] = command.update.todos
    assert is_binary(id)
    assert [%Message{role: :tool, tool_call_id: "call-todo"}] = command.update.messages
  end

  test "TODO tool validates actions and preserves state through commands" do
    todo = Todo.new()

    assert {:error, %{type: :invalid_todo_text}} =
             Tool.invoke(todo, %{"action" => "add", "text" => " "})
    ...
    assert [%{id: "todo-1", text: "draft plan", status: "open"}] = update.todos
    ...
    assert [%{id: "todo-1", text: "ship plan", status: "open"}] = update.todos
    ...
    assert [%{id: "todo-1", text: "ship plan", status: "complete"}] = update.todos

    assert {:error, %{type: :todo_not_found}} =
             Tool.invoke(todo, %{
               "action" => "update",
               "id" => "missing",
               "text" => "nope",
    ...
    assert {:error, %{type: :invalid_input}} =
             Tool.invoke(todo, %{"action" => "delete", "id" => "todo-1"})
  end
