defmodule BeamWeaver.Agent.TodoListMiddlewareTest do
  ...
  test "initializes with a native todo tool and default prompt" do
    ...
    assert middleware.system_prompt =~ "`write_todos`"
    assert [%BeamWeaver.Tools.Todo{} = tool] = TodoList.tools(middleware)
    assert Tool.name(tool) == "write_todos"
    assert Tool.description(tool) =~ "structured task list"
    assert Tool.description(tool) =~ "## When to Use This Tool"
  end
  ...
  test "agent installs the todo tool and updates todo state across tool calls" do
    ...
    model =
      model(table, [
        ...
        [
          todo_call("update", %{
            "todos" => [%{"content" => "Task 1 updated", "status" => "in_progress"}]
          })
        ],
        [
          todo_call("complete", %{
            "todos" => [%{"content" => "Task 1 updated", "status" => "completed"}]
          })
        ],
        []
      ])
    ...
    assert state.todos == [%{content: "Task 1 updated", status: "completed"}]
    ...
    assert Enum.map(Keyword.fetch!(opts, :tools), &Tool.name/1) == ["write_todos"]
  end
  ...
  test "custom todo middleware works in an agent" do
    ...
    model =
      model(table, [
        [
          todo_call("add", %{
            "todos" => [%{"content" => "Custom task", "status" => "in_progress"}]
          })
        ],
        []
      ])
    ...
    assert state.todos == [%{content: "Custom task", status: "in_progress"}]
    assert_received {:todo_model_call, 1, [%Message{role: :system, content: prompt} | _], opts}
  ...
  test "parallel todo tool calls are rejected with synthetic tool errors" do
    ...
    state = %{
      messages: [
        ...
        Message.assistant("",
          tool_calls: [
            todo_call("call-1", %{"todos" => [%{"content" => "Task 1", "status" => "pending"}]}),
            todo_call("call-2", %{"todos" => [%{"content" => "Task 2", "status" => "pending"}]})
          ]
        )
      ]
    }
  ...
  defp todo_call(id, args) do
    %{id: id, name: "write_todos", args: args}
  end
end
