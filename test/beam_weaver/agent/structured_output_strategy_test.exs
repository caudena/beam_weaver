defmodule BeamWeaver.Agent.StructuredOutputStrategyTest do
  ...
  defmodule InvalidProviderStructuredAgent do
    ...
    model(%FakeChatModel{structured_response: %{"age" => 37}, profile: %{structured_output: true}})
  ...
  test "Google provider strategy falls back to tool strategy for large schemas" do
    large_schema = %{
      "title" => "LargeResponse",
      "type" => "object",
      "properties" =>
    ...
    strategy =
      StructuredOutput.effective_strategy(
        StructuredOutput.provider(large_schema, name: "LargeResponse"),
        %{
          profile: %{
            provider: :google,
    ...
    assert %StructuredOutput.ToolStrategy{schema_specs: [spec]} = strategy
    assert spec.name == "LargeResponse"
  end

  test "Google provider strategy still uses native structured output for small schemas" do
    strategy =
      StructuredOutput.effective_strategy(
        StructuredOutput.provider(@person_schema, name: "PersonResponse"),
        %{
    ...
    assert %StructuredOutput.ProviderStrategy{} = strategy
  end

  test "structured-output policy reports requested and effective strategy metadata" do
    lookup_tool =
      Tool.from_function!(
        name: "lookup",
        description: "Lookup facts.",
    ...
    {strategy, policy} =
      StructuredOutput.effective_strategy_info(
        StructuredOutput.auto(@person_schema),
        %{profile: %{provider: :openai, structured_output: true}},
        [lookup_tool]
      )

    assert %StructuredOutput.ToolStrategy{} = strategy
    assert policy.requested_strategy == :auto
    assert policy.effective_strategy == :tool
    assert policy.fallback_reason == :tools_not_supported
    assert policy.schema_bytes > 0
    assert policy.schema_properties == 3
  end

  test "structured-output strategy follows provider profile policy fields" do
    lookup_tool =
      Tool.from_function!(
        name: "lookup",
        description: "Lookup facts.",
    ...
    assert {%StructuredOutput.ProviderStrategy{}, %{effective_strategy: :provider}} =
             StructuredOutput.effective_strategy_info(
               StructuredOutput.provider(@person_schema),
               %{
                 profile: %{
                   provider: :openai,
                   structured_output: true,
    ...
    assert {%StructuredOutput.ToolStrategy{}, %{fallback_reason: :tools_not_supported}} =
             StructuredOutput.effective_strategy_info(
               StructuredOutput.provider(@person_schema),
               %{profile: %{provider: :xai, structured_output: true}},
               [lookup_tool]
             )

    assert {%StructuredOutput.ToolStrategy{}, %{fallback_reason: :schema_too_many_properties}} =
             StructuredOutput.effective_strategy_info(
               StructuredOutput.provider(
                 %{
                   "title" => "LargeGemini",
                   "type" => "object",
                   "properties" => Map.new(1..90, &{"field_#{&1}", %{"type" => "string"}})
                 },
                 name: "LargeGemini"
               ),
               %{
    ...
    assert {%StructuredOutput.ToolStrategy{}, %{fallback_reason: :schema_too_large}} =
             StructuredOutput.effective_strategy_info(
               StructuredOutput.provider(@person_schema),
               %{
                 profile: %{
                   provider: :moonshot,
                   structured_output: true,
                   structured_output_max_schema_bytes: 10
                 }
               },
               []
             )
  ...
  test "schema specs can override tool names and descriptions for generated tools" do
    ...
    assert [%{name: "custom_tool_name", description: "Custom tool description", metadata: metadata}] =
             StructuredOutput.setup_tools(%StructuredOutput.ToolStrategy{
               schema: @person_schema,
               schema_specs: [spec]
             })

    assert metadata.structured_response == true
    assert metadata.structured_output_schema == "custom_tool_name"
  end
      ...
      Message.assistant("",
        tool_calls: [
          %{
            id: "call-todo",
            name: "write_todos",
            args: %{"todos" => [%{"content" => "Read context", "status" => "in_progress"}]}
          }
        ]
      )
