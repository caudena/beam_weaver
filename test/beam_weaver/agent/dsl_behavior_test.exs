defmodule BeamWeaver.Agent.DSLBehaviorTest do
  ...
  defmodule MixedStructuredToolModel do
    ...
    @impl true
    def invoke(%__MODULE__{parent: parent}, messages, opts) do
      tool_names = Enum.map(Keyword.get(opts, :tools, []), &Tool.name/1)

      if parent,
        do:
          send(
            parent,
            {:mixed_structured_model_call, Enum.map(messages, & &1.role), tool_names}
          )

      cond do
        "answer" in tool_names ->
          {:ok,
           Message.assistant("",
             tool_calls: [
               %{id: "call-answer", name: "answer", args: %{"value" => "ok"}}
        ...
        Enum.any?(messages, &match?(%Message{role: :tool, name: "regular_tool"}, &1)) ->
          {:ok, Message.assistant("ready for final structured response")}
  ...
  test "declared assistant name is set on assistant messages across model iterations" do
  ...
  test "model middleware wraps every model call in declared order" do
  ...
  test "structured output and regular tool calls execute regular tools before ending" do
    ...
    assert tool_names == ["regular_tool"]

    assert_receive {:mixed_structured_model_call, [:user, :assistant, :tool], tool_names}
    assert tool_names == ["regular_tool"]

    assert_receive {:mixed_structured_model_call, [:user, :assistant, :tool, :assistant], tool_names}
  ...
  test "provider structured output allows tool-call turns before final JSON" do
    ...
    assert [
             ...
             %Message{role: :assistant, content: "ready for final structured response"},
