defmodule BeamWeaver.Agent.MiddlewareBuiltinTest do
  ...
  alias BeamWeaver.Agent.Middleware.ToolChoice
  ...
  defmodule ToolChoiceAgent do
    ...
    middleware([
      {ToolChoice, choice: %{type: "function", name: "lookup"}, when_tool_present: "lookup"}
    ])
  ...
  defmodule ModelCallLimitMessageAgent do
    use BeamWeaver.Agent
    ...
    middleware([
      {BeamWeaver.Agent.Middleware.ModelCallLimit,
       thread_limit: 0, exit_behavior: :end, message: "No additional research needed."}
    ])
  end
  ...
  test "model call limit can end with a custom assistant message" do
    parent = self()

    assert {:ok, %{messages: messages}} =
             Agent.invoke(ModelCallLimitMessageAgent, %{messages: [Message.user("hello")]}, context: %{parent: parent})
    ...
    assert [
             ...
             %Message{
               role: :assistant,
               content: "No additional research needed.",
               metadata: %{error_type: :model_call_limit_exceeded}
             }
           ] = messages
  end
  ...
  test "public middleware enum options reject strings" do
    assert_raise ArgumentError, ~s(exit_behavior must be an atom, got "continue"; use :continue), fn ->
      ToolCallLimit.new(exit_behavior: "continue")
    end

    assert_raise ArgumentError, ~s(exit_behavior must be an atom, got "end"; use :end), fn ->
      ModelCallLimit.new(exit_behavior: "end")
    end

    assert_raise ArgumentError, ~s(on_failure must be an atom, got "continue"; use :continue), fn ->
      ModelRetry.new(on_failure: "continue")
    end

    assert_raise ArgumentError, ~s(on_failure must be an atom, got "return_message"), fn ->
      ToolRetry.new(on_failure: "return_message")
    end

    assert_raise ArgumentError, ~s(token_count_method must be an atom, got "model"; use :model), fn ->
      ContextEditing.new(token_count_method: "model")
    end
  end
  ...
  test "tool choice middleware forwards provider tool choice without filtering tools" do
    parent = self()

    assert {:ok, _state} =
             Agent.invoke(ToolChoiceAgent, %{messages: [Message.user("lookup")]}, context: %{parent: parent})

    assert_received {:model_call, _messages, opts}
    assert Keyword.fetch!(opts, :tool_choice) == %{type: "function", name: "lookup"}
    assert Enum.map(Keyword.fetch!(opts, :tools), &Tool.name/1) == ["lookup"]
  end
  ...
  test "summarization normalizes provider model strings when constructed" do
    middleware =
    ...
    assert %BeamWeaver.Google.ChatModel{model: "gemini-3.5-flash"} = middleware.model
  end

  test "subagent outputs middleware can end once required captured outputs are present" do
    middleware =
      BeamWeaver.Agent.Middleware.SubagentOutputs.new(
        required: [:first_output, :second_output],
        response: fn outputs ->
          %{
            "status" => "completed",
            "first" => Map.has_key?(outputs, "first_output"),
            "second" => Map.has_key?(outputs, "second_output")
          }
        end
      )
    ...
    assert {:jump, :end,
            %{
             ...
             BeamWeaver.Agent.Middleware.SubagentOutputs.before_model(
               middleware,
               %{subagent_outputs: %{:first_output => %{"ok" => true}, "second_output" => %{"ok" => true}}},
               nil
             )
