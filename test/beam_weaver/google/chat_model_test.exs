defmodule BeamWeaver.Google.ChatModelTest do
  ...
  test "request body defaults bound tools to Google AUTO mode" do
    tool =
      Tool.from_function!(
        name: "lookup",
        description: "Lookup a record",
    ...
    refute Map.has_key?(disabled_body, "toolConfig")
  end
  ...
  test "request body removes unsupported JSON Schema keywords from function declarations" do
    ...
    nullable_label = filters["properties"]["nullable_label"]
    ...
    assert nullable_label == %{"type" => "string", "nullable" => true}
  ...
  test "decoded Gemini function calls preserve thought signatures for replay without duplicating call blocks in content" do
    long_signature = String.duplicate("sig-a", 1_000)
    ...
    assert [%ToolCall{name: "lookup", thought_signature: ^long_signature}] = decoded.tool_calls
  ...
  test "stream_response reconstructs Gemini function calls from SSE parts" do
    ...
    assert {:ok, response} = ChatModel.stream_response(model, [Message.user("stream")], tools: [tool])
    assert Message.text(response) == ""
    assert response.status == "STOP"
    assert response.metadata.finish_message == "Model generated function call(s)."
