defmodule BeamWeaver.Transport.ReqFinchTest do
  ...
  test "returns status and body from a live HTTP response" do
    url = start_http_server("HTTP/1.1 200 OK\r\ncontent-length: 2\r\nconnection: close\r\n\r\nok")
    request = Request.new(method: :get, url: url)

    assert {:ok, %Response{status: 200, body: "ok"}} = ReqFinch.request(request, timeout: 1_000)
  end

  test "keeps HTTP error responses as responses for provider code to interpret" do
    url = start_http_server("HTTP/1.1 500 Internal Server Error\r\ncontent-length: 4\r\n\r\nfail")
    request = Request.new(method: :get, url: url)

    assert {:ok, %Response{status: 500, body: "fail"}} = ReqFinch.request(request, timeout: 1_000)
  end
