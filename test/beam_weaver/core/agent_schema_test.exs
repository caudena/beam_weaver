defmodule BeamWeaver.Core.AgentSchemaTest do
  ...
  test "agent actions preserve LangChain interop fields and replay assistant log messages" do
    ...
    assert AgentAction.lc_namespace() == ["langchain", "schema", "agent"]
  ...
  test "agent finish replays final assistant log and namespace" do
    finish = AgentFinish.new(%{"output" => "42"}, "Final Answer: 42")

    assert finish.type == "AgentFinish"
    assert AgentFinish.lc_namespace() == ["langchain", "schema", "agent"]
