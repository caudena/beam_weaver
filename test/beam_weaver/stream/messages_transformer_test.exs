defmodule BeamWeaver.Stream.MessagesTransformerTest do
  ...
  # Upstream references:
  # - langgraph/libs/langgraph/tests/test_stream_messages_transformer.py::TestProtocolEventRouting
  # - langgraph/libs/langgraph/tests/test_stream_messages_transformer.py::TestWholeMessageFallback
  # - langgraph/libs/langgraph/tests/test_stream_messages_transformer.py::TestFiltering
  # - langgraph/libs/langgraph/tests/test_stream_messages_transformer.py::TestLifecycle
  # - langgraph/libs/langgraph/tests/test_stream_messages_transformer.py::TestAsyncMode
  # - langgraph/libs/langgraph/tests/test_stream_messages_transformer.py::TestViaMux
  ...
  test "typed BeamWeaver message envelopes can be projected as whole messages" do
    envelope = %Envelope{
      event: %Events.Message{
        ...
        metadata: %{langgraph_node: "model", run_id: "run-typed"}
      },
      node: "model",
      run_id: "run-typed"
    }
  ...
  defp proto(payload, opts \\ []) do
    ...
    %{
      ...
      "params" => %{
        "namespace" => namespace,
        "timestamp" => 1,
        "data" => {payload, %{"langgraph_node" => node, "run_id" => run_id}}
      }
    }
  end
    ...
    %{
      ...
      "params" => %{
        ...
        "data" =>
          {message,
           %{
             "langgraph_node" => Keyword.get(opts, :node, "node"),
