  ...
  for adapter <- [:ets, :ecto] do
    describe "#{adapter} memory store conformance" do
      ...
      test "validates public namespaces without blocking internal batch writes", %{store: store} do
        ...
        assert {:error, {:invalid_namespace, _}} = Memory.put(store, ["langgraph"], "key", %{})

        assert [nil] =
                 Memory.batch(store, [
                   %PutOp{namespace: ["langgraph"], key: "internal", value: %{"ok" => true}}
                 ])

        assert [%{value: %{"ok" => true}}] =
                 Memory.batch(store, [%GetOp{namespace: ["langgraph"], key: "internal"}])
      end
      ...
      test "base-store batch key helpers preserve order, idempotency, missing keys, and prefix listing",
           %{store: store} do
        # Upstream references:
        # - langchain/libs/core/tests/unit_tests/stores/test_in_memory.py
        # - langchain/libs/standard-tests/langchain_tests/integration_tests/base_store.py
