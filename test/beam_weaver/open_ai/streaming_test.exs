defmodule BeamWeaver.OpenAI.StreamingTest do
  ...
  test "typed events tolerate in-memory prompt cache retention literal drift" do
    # Upstream reference:
    # langchain/libs/partners/openai/tests/unit_tests/chat_models/test_responses_stream.py
    body = """
  ...
  test "Responses usage-only completion preserves nested and unknown usage metadata" do
    ...
    assert BeamWeaver.Stream.to_langsmith_event(done).token_usage["provider_new_usage"] == %{
             "kept" => true
           }
  end
  ...
  test "chat-completions usage-only chunks preserve unknown usage fields for tracing" do
    ...
    assert BeamWeaver.Stream.to_langsmith_event(done).token_usage["vendor_token_breakdown"] ==
             %{"audio_tokens" => 1}
  end
