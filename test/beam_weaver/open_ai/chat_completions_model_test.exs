defmodule BeamWeaver.OpenAI.ChatCompletionsModelTest do
  ...
  test "builds Chat Completions requests with tools, structured output, multimodal options, and escape hatches" do
    # Upstream reference:
    # langchain/libs/core/tests/unit_tests/messages/block_translators/test_openai.py
    # standard-tests/langchain_tests/unit_tests/chat_models.py
  ...
  test "o-series Chat Completions requests coerce system role to developer" do
    # Upstream reference:
    # langchain/libs/partners/openai/tests/unit_tests/chat_models/test_base.py::test__get_request_payload
    assert {:ok, body} =
             ChatCompletionsModel.request_body(
  ...
  test "event streams carry model metadata for telemetry and LangSmith mapping" do
    request_body = %{
      "model" => "gpt-5.4-mini",
    ...
    done = Enum.find(events, &match?(%Envelope{event: %Events.Done{}}, &1))

    assert BeamWeaver.Stream.to_langsmith_event(done).token_usage["prompt_tokens_details"] == %{
             "cached_tokens" => 1
           }
  end
