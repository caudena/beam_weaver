...
building your own harness preset or compatibility layer and want serializable
profile data that mirrors Deep Agents' harness profile shape.
...
overlay. Direct agent options remain the runtime source of truth for harness
behavior.
{% endhint %}

## Capability Profiles

`BeamWeaver.Agent.CapabilityProfile` is the BeamWeaver equivalent of the
official Deep Agents harness profile. It stores provider/model-specific harness
preferences:
...
| Field | Meaning |
| --- | --- |
| `:base_system_prompt` | Replacement base prompt text for a custom harness. |
...
For normal BeamWeaver agents, configure harness behavior directly on the agent:
...
Model profiles are not Deep Agents harness profiles. They are BeamWeaver's
