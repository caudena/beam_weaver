...
Module-defined agents use the `async_subagents` DSL:
...
`AsyncSpec.new/1` accepts keyword lists, atom-keyed maps, and string-keyed
maps. If `:url` is present and `:client` is omitted, it selects
...
| `:subagent_name` / `:agent_name` | Async subagent name. |
...
Custom clients are the right place to implement LangSmith-specific auth,
internal service discovery, retries, telemetry, or non-HTTP transports.
...
| LangSmith Deployment auth via `LANGSMITH_API_KEY` / `LANGGRAPH_API_KEY` | Not built into `ReqClient`. Pass `headers:` or implement a client. |
| `langgraph dev --n-jobs-per-worker` | Not applicable to BeamWeaver. Size the worker runtime you actually use. |
| LangSmith trace correlation by thread ID | BeamWeaver keeps task IDs, thread IDs, and run IDs in `:async_tasks`; export/correlation is up to the tracing integration or custom client. |
