...
Synchronous BeamWeaver subagents inherit parent skills when the subagent spec
leaves `skills` as `nil`. Set `skills: []` to opt out, or provide a list to
replace the inherited sources:
