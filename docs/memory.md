...
BeamWeaver does not implement LangSmith cron jobs or `langgraph.json`. Use your
...
| LangSmith `rt.server_info.user.identity` / `assistant_id` | Use `runtime.context`, `runtime.server_info`, or application-provided context depending on deployment. BeamWeaver does not provide LangSmith identity automatically. |
...
| LangSmith cron jobs for background consolidation | Not built in. Use your application scheduler and invoke a normal BeamWeaver agent. |
| LangSmith Store API seeding | Seed with `BeamWeaver.Memory.put/5`, `BeamWeaver.Filesystem.write/4`, migrations, or application admin workflows. |
...
| LangSmith Agent Server automatic persistence | Not built in. Applications pass explicit checkpointer and store adapters. |
