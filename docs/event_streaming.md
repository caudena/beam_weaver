...
| `ProtocolEvent` dictionaries as the main public shape. | BeamWeaver's public shape is `%BeamWeaver.Stream.Envelope{event: typed_struct}`. `BeamWeaver.Stream.to_langsmith_event/1` can export a LangSmith-like map when needed. |
...
| LangSmith Agent Server streaming. | BeamWeaver does not implement LangSmith Agent Server. Use in-process streams or build an application transport around typed envelopes. |
