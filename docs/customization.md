...
harness behavior is documented in [Agent Harness](agent_harness.md).
{% endhint %}
...
```elixir
tools [MyApp.Tools.SearchDocs, MyApp.Tools.CreateTicket]
```
...
middleware, or profiles instead of copying the built-in harness prompt into
every agent.
...
```elixir
middleware [
  {BeamWeaver.Agent.Middleware.ModelRetry, max_attempts: 3},
...
[Agent Harness](agent_harness.md#code-execution).
...
- Capability profiles package harness-level defaults for custom harness code.

See [Profiles](profiles.md).

### Structured Output

Use `response_format` for the agent's final structured result:

```elixir
response_format BeamWeaver.Agent.StructuredOutput.tool(MyApp.Schema.contact())
```
...
- Capability profiles mirror Deep Agents harness profile data for custom
  harness code, but normal agent builds do not automatically overlay them.
...
| Automatic LangSmith deployment infrastructure | Configure checkpointers, stores, cache, tracing exporters, auth boundaries, and sandbox lifecycle in Elixir. |
