...
For static agent structured output, use `response_format/1` in the agent module.
See [Structured Output](structured_output.md) for provider and tool strategies.

{% hint style="warning" %}
**No Pydantic Or TypedDict Schemas**

Python examples use Pydantic models and `TypedDict`. BeamWeaver uses JSON
Schema-shaped maps and BeamWeaver structured-output strategy structs. Elixir
structs and typespecs are not enough to describe provider-facing schema
validation.
{% endhint %}
...
tools and instructions, and have subagents write large artifacts to the
filesystem when the parent may need to inspect details later.
...
| LangSmith Store API pre-seeding | Use `BeamWeaver.Memory` adapters or your application's store setup path. |
