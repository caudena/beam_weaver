...
Synchronous subagents inherit the parent agent's filesystem permissions by
default. Set `:permissions` on the subagent spec to replace the inherited list:
...
`permissions: []`. To avoid exposing filesystem tools at all, give the subagent
a filesystem-free compiled agent or remove the filesystem capability from that
agent.
