...
LangSmith Agent Server. Production is your Elixir application: OTP supervision,
...
thread APIs, or automatic LangSmith deployment infrastructure. Integrate those
...
| --- | --- |
| Managed Deep Agents, LangSmith Deployments, Agent Server, hosted threads, hosted runs, and `langgraph.json`. | Not built in. Run BeamWeaver inside your own Elixir/OTP application and expose your chosen API. |
| LangSmith custom auth and authorization handlers. | Use your application's auth layer and pass trusted identity through `context:` or `server_info:`. |
| Agent Auth OAuth flow. | Not built in. Implement OAuth in your application or host-side tools. |
| LangSmith workspace secrets and sandbox auth proxy. | Not built in. Use your deployment secret manager and keep secrets out of model-visible state and sandboxes. |
...
| LangSmith cron jobs for memory consolidation. | Use Oban, Quantum, Kubernetes CronJobs, or another application scheduler. |
