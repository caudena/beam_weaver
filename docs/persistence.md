...
LangGraph's persistence docs include LangSmith Agent Server behavior where
...
| --- | --- |
| LangSmith Agent Server automatically handles checkpoint/store infrastructure. | Not built in. Configure adapters explicitly. |
...
| LangSmith semantic-search store config in `langgraph.json`. | Not supported. Configure BeamWeaver memory adapters in Elixir. |
