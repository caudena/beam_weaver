...
BeamWeaver uses the same graph idea with Elixir data and functions.
...
calculator =
  Graph.new(name: "Calculator")
  |> Graph.add_node(:route, fn state -> %{operation: state.operation} end)
  |> Graph.add_node(:add, fn state -> %{result: state.a + state.b} end)
  |> Graph.add_node(:multiply, fn state -> %{result: state.a * state.b} end)
  |> Graph.add_node(:divide, fn state -> %{result: state.a / state.b} end)
  |> Graph.add_edge(Graph.start(), :route)
  |> Graph.add_edge(:route, :add, when: %{operation: :add})
  |> Graph.add_edge(:route, :multiply, when: %{operation: :multiply})
  |> Graph.add_edge(:route, :divide, when: %{operation: :divide})
  |> Graph.add_edge(:add, Graph.end_node())
  |> Graph.add_edge(:multiply, Graph.end_node())
  |> Graph.add_edge(:divide, Graph.end_node())
  |> Graph.compile!()

{:ok, state} =
  Compiled.invoke(calculator, %{
    operation: :add,
    a: 3,
    b: 4
  })
...
```elixir
calculator
|> Compiled.stream_events(%{operation: :multiply, a: 6, b: 7})
...
not expose a separate `create_deep_agent` constructor. Instead, those harness
capabilities are normal agent options and middleware on `BeamWeaver.Agent`.
...
Use [Agent Harness](agent_harness.md) for the full capability map.
{% endhint %}
...
LangChain's quickstarts use LangSmith hosted tracing and deployment links.
BeamWeaver has telemetry and tracing/export boundaries, but it does not
implement LangSmith Engine, LangSmith Deployment, Studio, or remote LangGraph
Platform APIs as built-in product features.
{% endhint %}
